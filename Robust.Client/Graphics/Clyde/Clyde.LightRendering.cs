    ...
    internal partial class Clyde
    {
        ...
        private void ApplyFovToBuffer(Viewport viewport, IEye eye)
        {
            ...
            if (fovBlurMult > 0f && viewport.FovMaskTarget != null!)
            {
                ...
                // Structura (owner 2026-09-08/09 «пропала зона заднего FOV», «всё вокруг цветное»): вернуть в UBO
                // МИРОВЫЕ матрицы. Композит выше рисуется в экранных координатах, а SetProjViewBuffer (в отличие от
                // SetProjViewFull) правит только буфер GPU и НЕ трогает кэш _currentMatrix* — поэтому кэш всё ещё
                // мировой, а буфер остаётся экранным. Сразу после нас Clyde.HLR рисует оверлеи OverlaySpace.WorldSpace
                // (слепая зона за спиной, кровопотеря, лучи света, VHS): их полноэкранный квад задаётся в МИРОВЫХ
                // координатах и с экранными матрицами улетал за пределы кадра — эффекты просто исчезали. Стоковая
                // ветка ниже матриц не трогает и потому не течёт; течёт только наш блюр-путь (§7.9). Приём и
                // комментарий — как у соседнего BlurRenderTarget: «кэш не топтали, просто откатываем буфер».
                SetProjViewBuffer(_currentMatrixProj, _currentMatrixView);
                return;
            }
